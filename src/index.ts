...
export class WebsiteScope {
  ...
  createTeamWorkflow<T = unknown>(body: { name: string; trigger: Record<string, unknown>; steps?: unknown[]; description?: string }): Promise<T> {
    return this.request('POST', '/team-chat/workflows', { body });
  }
  /** Partial patch; a field you do not send does not change. `published: true` publishes. */
  updateTeamWorkflow<T = unknown>(workflowId: string, body: Record<string, unknown>): Promise<T> {
    return this.request('PATCH', `/team-chat/workflows/${encodeURIComponent(workflowId)}`, { body });
  }
  /** Deletes the workflow; its trigger goes with it (the trigger is a field, not an object). */
  deleteTeamWorkflow<T = unknown>(workflowId: string): Promise<T> {
    return this.request('DELETE', `/team-chat/workflows/${encodeURIComponent(workflowId)}`);
  }
  /** Reads who a canvas is open to, and at which level — the twin of the list endpoint, so what you know about one is true of the other. The access level (`invite_only` / `org_view` / `org_edit`) is a property of the CANVAS, not of a share row: "everyone in the workspace can see it" has no target. `only_owner_can_share` tells you whether the creator limited sharing to themselves — read it before you try, rather than discovering it as a 403. */
  getTeamCanvasShares<T = unknown>(docId: string): Promise<T> {
    return this.request('GET', `/team-chat/docs/${encodeURIComponent(docId)}/shares`);
  }
  /** Removes a comment from a canvas. Your app could already WRITE comments; not being able to remove them was a one-way authority. An app has no "own" comments (there is no person behind an API key), so the rule that applies to you is the other half of the product's: you need EDIT access on the canvas. A comment id that is not on this canvas answers 404 rather than being looked up globally. */
  deleteTeamCanvasComment<T = unknown>(docId: string, commentId: string): Promise<T> {
    return this.request('DELETE', `/team-chat/docs/${encodeURIComponent(docId)}/comments/${encodeURIComponent(commentId)}`);
  }
  ...
  deleteTeamList<T = unknown>(listId: string): Promise<T> {
    return this.request('DELETE', `/team-chat/lists/${encodeURIComponent(listId)}`);
  }
  /**
   * Creates a NEW list from a CSV body — never adds columns to an existing table. Parsing happens server-side (RFC 4180: quoted commas and newlines, doubled quotes, BOM, CRLF), so a client that splits on commas cannot produce a different table than the API does. Column types are inferred conservatively — a column is checkbox/number/date/email only when EVERY non-empty cell matches, otherwise text; `select` is never guessed. Rows whose column count does not match are skipped and returned in `issues` with their FILE line number. Caps: 10 MB, 5000 rows.
   */
  importTeamListCsv<T = unknown>(body: { title: string; csv: string; has_header?: boolean }): Promise<T> {
    return this.request('POST', '/team-chat/lists/import', { body });
  }
  /**
